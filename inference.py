    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPosition in self.legalPositions:
            state = self.setGhostPosition(gameState, oldPosition)
            belief = self.beliefs[oldPosition]
            newPosDist = self.getPositionDistribution(state)
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += belief * prob
